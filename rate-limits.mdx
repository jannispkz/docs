...
<Info>
  **Current Limit:** Maximum of **10 concurrent requests** per API key
</Info>
...
```
Time 0s: Send 10 requests → All accepted ✓
Time 1s: Request #1 completes → Can send 1 more
Time 2s: Requests #2-4 complete → Can send 3 more
Time 3s: Send 4 requests → 3 accepted, 1 rejected (at limit)
```
...
If you consistently need more than 10 concurrent requests:

- Contact [support@pikzels.com](mailto:support@pikzels.com)
- Describe your use case and expected volume
- Ask about enterprise plans with custom limits
